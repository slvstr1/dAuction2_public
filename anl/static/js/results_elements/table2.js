...
var Table_main2 = React.createClass({
    render() {
        ...
        for (var key in rows) {
            if (rows.hasOwnProperty(key)) {
                ...
                var pe_spot = this.props.treatment.price_avg_theory
                var fp_avg = Math.round(average(price_forward) - pe_spot)
                var fp_med = Math.round(getMedian(price_forward)  - pe_spot)
                rows[key].pe_spot = pe_spot
                rows[key].fp = String(fp_avg) + "/" + String(fp_med);
            }
        }
        ...
        var Table_head = <tr>
            ...
            <th scope="col">Pexp spot</th>
            <th scope="col">FP (avg/med)</th>
        </tr>
        return (
            <div>
...
window.Table_main2 = Table_main2;
