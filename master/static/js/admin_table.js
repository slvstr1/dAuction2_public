...
var Player_table_row = React.createClass({
    render: function () {
        if (this.props.last_alive > 50) {
            var divStyle = {
                color: 'red',
                fontWeight: "bold"
            }
        } else if (this.props.last_alive > 20) {
            var divStyle = {
                color: 'orange',
                fontWeight: "bold"
            }
    ...
    fetchPlayerData: function () {
        $.get("/ajax_admin", function (data) {
            if (this.isMounted()) {
                ...
                window.UNIQUEIZER = data.UNIQUEIZER
                this.setState({
    ...
    render: function () {
        ...
        var tablerows = manipulate_sorted.map(function (player) {
            ...
            return (<Player_table_row
                    ...
                    auction={player.auction_id.toLocaleString('de-DE')}
                    group={player.group_id - (UNIQUEIZER * player.auction_id)}
                    ip={player.user.ip}
                    role={player.role}
