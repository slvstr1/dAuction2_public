// PLOT with market situation
// uses all_offers={this.state.all_offers} plot_w={plot_w} plot_l={plot_h} timestamp={timest}
// computes curves from provided data about offers
// used function
...
var MarketPlot = React.createClass({
  render () {
      ...
      // if nonempty, calculate
          ...
          var offers_copy1 = JSON.parse(JSON.stringify(this.props.all_offers));
          ...
          var offers_copy2 = JSON.parse(JSON.stringify(this.props.all_offers));
          ...
          for (var key in sorted_offers) {
              if (sorted_offers.hasOwnProperty(key)) {
                  // if not cleared
                  if (sorted_offers[key].cleared == false) {
          ...
          // sum all quantities
          var demand_unitsS = [], demandR_unitsS = [], supply_unitsS = [], supplyR_unitsS = [], transactions_unitsS = [];
          ...
          var demand_unitsSP = doubleX(demand_unitsS), demandR_unitsSP = doubleX(demandR_unitsS), supply_unitsSP = doubleX(supply_unitsS), supplyR_unitsSP = doubleX(supplyR_unitsS), transactions_unitsSP = doubleX(transactions_unitsS);
          // doubleY to prices
          var demand_priceP = doubleY(demand_price), supply_priceP = doubleY(supply_price), demandR_priceP = doubleY(demandR_price), supplyR_priceP = doubleY(supplyR_price), transactions_priceP = doubleY(transactions_price);
              ...
              return supply_unitsSP[i] + transactions_unitsSP[transactions_unitsSP.length - 1]
                   ...
                   // main supply
                    ...
                    var layout = {
                        ...
                        title: "Offers and Transactions Graph"
                    };
      var data1 = [trace1, trace2, trace3, trace4, trace5];
    return (
        <div id="testeeeee"
             style={{margin: "auto", position: "fixed", bottom: "42%", top: "5.8%", left: "42%", right:"2%"}}>
            <RP id="myDivee"
                style={{margin: "auto", position: "fixed", bottom: "42%", top: "5.8%", left: "42%", right:"2%"}}
